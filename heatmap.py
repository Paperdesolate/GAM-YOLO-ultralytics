...
def get_params():
    params = {
        'weight': 'D:/ultralytics/8m-segbest.pt ',  # 训练出来的权重文件
        'cfg': 'D:/ultralytics/ultralytics/cfg/models/v8/yolov8-seg.yaml',  # 训练权重对应的yaml配置文件
        'device': 'cuda:0',
        ...
        'conf_threshold': 0.11,  # 0.6  # 置信度阈值，有的时候你的进度条到一半就停止了就是因为没有高于此值的了
        'ratio': 0.03 # 0.02-0.1
    }
    return params
   ...
   # model(r'D:/mydata/un', r'D:/mydata/un1')  # 第一个是检测的文件, 第二个是保存的路径

if __name__ == '__main__':
    model = yolov8_heatmap(**get_params())

    input_dir = r'D:/mydata/un'
    output_dir = r'D:/mydata/un1'
    ...
    for filename in os.listdir(input_dir):
        if filename.lower().endswith(('.jpg', '.png', '.jpeg')):  # Check for valid image formats
            img_path = os.path.join(input_dir, filename)  # Create full path to the image
            print(f"Processing: {img_path}")  # Debug line to check which file is being processed
            model(img_path, output_dir)  # Process each image
